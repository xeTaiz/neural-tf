...
from pl_module_tex import NeuralTransferFunction
...
if __name__=='__main__':
    ...
    parser.add_argument('checkpoint', type=str, help='Path to checkpoint with pre-trained weights')
    ...
    parser.add_argument('--max_epochs', default=10, type=int, help='Maximum number ob epochs to train')
    ...
    ckpt['hyper_parameters']['pretrained'] = False
    ckpt['hyper_parameters']['trainds'] = args.trainds
    ckpt['hyper_parameters']['one_vol'] = True
    ckpt['hyper_parameters']['finetune'] = True
    ckpt['hyper_parameters']['lr_im_backbone'] = 0
    ckpt['hyper_parameters']['lr_neural_tf'] = 1e-5
    ...
    model = NeuralTransferFunction(Namespace(**ckpt['hyper_parameters']))
    model.load_state_dict(ckpt['state_dict']) # Must be AFTER prepare_data(), because it messes with multiprocessing
    print(model)
    run_id = str(uuid.uuid4())[:6]
    logger = loggers.WandbLogger(
            project='deep-transfer-function',
            name=f'NEURALTF_TEX_FT_{run_id}_{args.n}',
            id=run_id,
    ...
    # Save source code !
    trainer.logger.experiment.save('pl_train_tex.py')
    trainer.logger.experiment.save('pl_module_tex.py')
    ...
    # Freeze backbone
    for p in model.im_backbone.parameters(): p.requires_grad = False
